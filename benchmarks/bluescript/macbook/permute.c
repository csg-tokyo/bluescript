...
static void fbody_permute(value_t _v, int32_t _n) {
  ...
  func_rootset.values[0] = _v;
  {
    ...
    if (_n != 0) {
      int32_t _n1 = _n - 1;
      _permute.fptr(func_rootset.values[1]=func_rootset.values[0], _n1);
      for (
...
struct _permute _permute = { fbody_permute, "([ii)v" };
