...
void test_array_push() {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        Assert_equals(gc_fixarray_length(arrvec), real_len);
        Assert_equals(value_to_ptr(arrvec)->body[0], real_len);
        for (int j = 0; j < i; j++)
            Assert_equals(*fast_fixarray_get(arrvec, j), int_to_value(i));
        for (int j = i; j < real_len; ++j)
            Assert_equals(*fast_fixarray_get(arrvec, j), VALUE_UNDEF);
        if (i > 0)
        ...
        for (int j = 0; j < 10; j++) {
            ...
            if (i > 0)
                Assert_equals(*fast_fixarray_get(vec, 0), int_to_value(70 + i));
            for (int k = 1; k < i; k++)
                Assert_equals(*fast_fixarray_get(vec, k), int_to_value(i));

            Assert_true(len >= i + j + 1);
            Assert_equals(*fast_fixarray_get(vec, i + j), int_to_value(90 + j));
            for (int k = i + j + 1; k < len; k++)
                Assert_equals(*fast_fixarray_get(vec, k), VALUE_UNDEF);
        }
    }
    ...
    for (int i = 0; i < 16; i++) {
        ...
        for (int j = 0; j < i; j++) {
            ...
            for (int k = 0; k < i - j - 1; k++)
                Assert_equals(*fast_fixarray_get(vec, k), int_to_value(100 + k));
            Assert_true(len >= i - j - 1);
            for (int k = i - j - 1; k < len; k++)
                Assert_equals(*fast_fixarray_get(vec, k), VALUE_UNDEF);
        }
...
void test_array_unshift() {
    for (int i = 0; i < 16; i++) {
        ...
        for (int j = 0; j < 10; j++) {
            ...
            int len = value_to_ptr(vec)->body[0];
            for (int k = 0; k < j + 1; k++)
                Assert_equals(*fast_fixarray_get(vec, k), int_to_value(90 + j - k));

            for (int k = j + 1; k < i + j + 1; k++)
                Assert_equals(*fast_fixarray_get(vec, k), int_to_value(100 + k - j - 1));
