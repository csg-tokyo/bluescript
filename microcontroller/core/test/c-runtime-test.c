...
static value_t gc_new_fixarray2(int32_t n) {
    return gc_new_fixarray(n, VALUE_UNDEF);
}

static value_t gc_new_array3(int32_t n) {
    ROOT_SET(root_set, 1)
    root_set.values[0] = gc_new_fixarray2(n);
    value_t obj = gc_new_fixarray2(1);
    gc_fixarray_set(obj, 0, root_set.values[0]);
    DELETE_ROOT_SET(root_set)
    return obj;
}

static value_t gc_array3_get(value_t obj, int32_t index) {
    return gc_fixarray_get(gc_fixarray_get(obj, 0), index);
}

static void gc_array3_set(value_t obj, int32_t index, value_t value) {
    gc_fixarray_set(gc_fixarray_get(obj, 0), index, value);
}
...
void test_String() {
    ...
    check_string_length(gc_new_fixarray(3, VALUE_NULL));
}
...
static value_t gc_fixarray_setter(value_t obj, value_t index, value_t new_value) {
    return gc_fixarray_set(obj, value_to_int(index), new_value);
}
...
void test_copy_array() {
    ...
    // Test 3: Copy from fixarray (fixed length array)
    value_t vec = gc_new_fixarray(3, VALUE_UNDEF);
    root_set.values[0] = vec;
    gc_fixarray_set(vec, 0, int_to_value(5));
    gc_fixarray_set(vec, 1, int_to_value(15));
    gc_fixarray_set(vec, 2, int_to_value(25));
...
void test_fixarray() {
    value_t arr = gc_new_fixarray2(4);
    value_t arr2 = gc_new_fixarray2(4);
    for (int i = 0; i < 4; i++)
        Assert_equals(gc_fixarray_setter(arr2, int_to_value(i), int_to_value(i)), int_to_value(i));
    for (int i = 0; i < 4; i++)
        gc_fixarray_setter(arr, i, int_to_value(i));
    for (int i = 0; i < 4; i++) {
        value_t e = gc_fixarray_get(arr2, int_to_value(i));
...
void test_allocate_heap() {
    ...
    for (int i = 0; i < 1024; i++) {
        value_t arr = gc_new_fixarray2(vec_size - 2);
...
void test_nested_root_set2() {
    ROOT_SET(root_set, 3);
    root_set.values[0] = gc_new_fixarray2(1);
    DELETE_ROOT_SET(root_set);
}
...
void test_gc_liveness2() {
    ...
    root_set.values[0] = obj = gc_new_fixarray2(4);
    obj4 = gc_new_fixarray2(1);
    gc_fixarray_setter(obj, int_to_value(0), obj2 = gc_new_bytearray(true, 8, 0));
    gc_bytearray_set_raw_word(obj2, 0, obj4);
    gc_new_fixarray2(1);
    gc_new_fixarray2(1);
    gc_fixarray_setter(obj, int_to_value(1), obj3 = gc_new_fixarray2(2));
    gc_fixarray_setter(obj3, int_to_value(0), obj);
    root_set.values[1] = gc_new_string("test");
...
void test_gc_sweep() {
    ...
    gc_new_fixarray2(2);
    gc_new_fixarray2(3);
    root_set.values[3] = obj = gc_new_fixarray2(4);
    obj2 = gc_new_fixarray2(3);
    gc_fixarray_setter(obj, 0, obj2);
