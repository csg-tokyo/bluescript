...
static void* mapped_iflash_address = NULL;
static void* mapped_dflash_address = NULL;
...
static void iram_init() {
    ...
    ESP_LOGI(BS_MEMORY_TAG, "IRAM Address: %p Size: %d\n", iram, (int)iram_size);
}
...
static void dram_init() {
    ...
    ESP_LOGI(BS_MEMORY_TAG, "DRAM Address: %p Size: %d\n", dram, (int)dram_size);
}
...
void* bs_memory_iflash_address() {
    return mapped_iflash_address;
}

void* bs_memory_dflash_address() {
    return mapped_dflash_address;
}
...
uint32_t bs_memory_iflash_size() {
    return iflash_partition->size;
}

uint32_t bs_memory_dflash_size() {
    return dflash_partition->size;
}


void bs_memory_iram_memcpy(void* dest, void *src, uint32_t len) {
    if ((uint8_t*)iram_address <= (uint8_t*)dest && dest < (uint8_t*)iram_address + iram_size) {
        memcpy(dest, src, len);
    } else {
        ESP_LOGE(BS_MEMORY_TAG, "The destination address: %p is out of IRAM range.", dest);
    }
}

void bs_memory_dram_memcpy(void* dest, void *src, uint32_t len) {
    if ((uint8_t*)dram_address <= (uint8_t*)dest && dest < (uint8_t*)dram_address + dram_size) {
        memcpy(dest, src, len);
    } else {
        ESP_LOGE(BS_MEMORY_TAG, "The destination address: %p is out of DRAM range.", dest);
    }
}

void bs_memory_iflash_memcpy(void* dest, void *src, uint32_t len) {
    if ((uint8_t*)mapped_iflash_address <= (uint8_t*)dest && dest < (uint8_t*)mapped_iflash_address + iflash_partition->size) {
        void* offset = dest - mapped_iflash_address;
        esp_partition_write(iflash_partition, offset, src, len);
    } else {
        ESP_LOGI(BS_MEMORY_TAG, "The destination address: %p is out of IFLASH range.", dest);
    }
}

void bs_memory_dflash_memcpy(void* dest, void *src, uint32_t len) {
    if ((uint8_t*)mapped_dflash_address <= (uint8_t*)dest && dest < (uint8_t*)mapped_dflash_address + dflash_partition->size) {
        void* offset = dest - mapped_dflash_address;
