// following lines should be commented out.
type integer = number;
type float = number;
...
function nbody_main() {
  ...
  const iterations: integer = 10000;

  for (let i = 0; i < iterations; i++) {
    advance(bodies, 0.01);
  }

  console_log_float(bodies[0][x])
  console_log_float(bodies[0][y])
  console_log_float(bodies[0][z])
  console_log_float(bodies[0][vx])
  console_log_float(bodies[0][vy])
  console_log_float(bodies[0][vz])
  console_log_float(bodies[0][mass])
