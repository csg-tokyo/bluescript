...
export const DISPLAY_HEART = 0;
export const DISPLAY_SMALL_HEART = 1;
export const DISPLAY_HAPPY = 2;
export const DISPLAY_SAD = 3;
...
export const displayFill = (color: integer) => {
    code`
    display_draw(fill, _color, _color);
    `
}

export const displayShowIcon = (icon: integer, color: integer, background: integer) => {
    if (icon === DISPLAY_HEART) {
        code`
        display_draw(heart, _color, _background);
        `
    } else if (icon == DISPLAY_SMALL_HEART) {
        code`
        display_draw(small_heart, _color, _background);
        `
    } else if (icon === DISPLAY_HAPPY) {
        code`
        display_draw(happy_face, _color, _background);
        `

    } else if (icon === DISPLAY_SAD) {
        code`
...
export const displayShowString = (str: string, color: integer, background: integer) => {
    code`
...
export const displayShowInt = (num: integer, color: integer, background: integer) => {
    code`
    char buff[12]; // Max num length + blank
    sprintf(buff, "%d", _num);
    display_text(&buff, _color, _background);
    `
}

export const getColor = (r: integer, g: integer, b: integer) => {
    let color: integer = 0;
    code`
    _color |= ((_r >> 3) << 11);
    _color |= ((_g >> 2) << 5);
...
export const randInt = (min: integer, max: integer) => {
    let ri = 0;
    code`
